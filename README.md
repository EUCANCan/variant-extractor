...
- [Homogenization rules](#homogenization-rules)
  - [SNVs](#snvs)
  - [Standard INSs and DELs](#standard-inss-and-dels)
...
### Standard INSs and DELs
All entries with the `REF/ALT` of different lengths are treated as either INS or DEL. If the `REF` sequence is longer than the `ALT` sequence, it is considered a deletion. If the `REF` sequence is shorter than the `ALT` sequence, it is considered an insertion. For example:

| CHROM | POS | ID      | REF | ALT | FILTER | [`VariantType`](#varianttype) |
| ----- | --- | ------- | --- | --- | ------ | ----------------------------- |
| 2     | 3   | event_1 | T   | TGT | PASS   | INS                           |
| 2     | 10  | event_2 | TA  | AAT | PASS   | INS                           |
| 2     | 5   | event_3 | TAG | T   | PASS   | DEL                           |
| 2     | 8   | event_4 | TAG | AG  | PASS   | DEL                           |
